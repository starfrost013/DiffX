...
public class DosExecutableHeader
{
    ...
    /// <summary>
    /// Alternative DOS signature used by some early compilers.
    /// </summary>
    public const ushort IMAGE_DOS_ALTERNATE_SIGNATURE = 0x4D5A; // ZM (endianness :O)
    ...
    public DosExecutableHeader(ReadOnlySpan<Byte> bytes)
    {
        ...
        if (Magic is not IMAGE_DOS_SIGNATURE)
            throw new InvalidDataException("Non-MZ header found");
    }
}